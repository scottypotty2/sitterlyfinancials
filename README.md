...
This is a modern web application built with React, TypeScript, Vite, TanStack Router, and Tailwind CSS. It is configured for easy development, testing, and deployment to Cloudflare using Wrangler.

# Prerequisites

- Node.js (v18 or higher recommended)
- npm (v9 or higher recommended)
- [Wrangler CLI](https://developers.cloudflare.com/workers/wrangler/) (installed automatically if not present)
...
```bash
npm install
npm run start  # Starts the development server on port 3000
```

# Deployment

This project uses [Cloudflare Wrangler](https://developers.cloudflare.com/workers/wrangler/) for deployment.
...
1. **Build and deploy locally:**
   ...
   npm run build
   ```
   - This will build your site and automatically deploy it using Wrangler (thanks to the `postbuild` script in `package.json`).
   - **Do not run `npx wrangler deploy` directly** unless you have already built the site and the `dist` directory exists.

2. **CI/CD or Hosting Platforms:**
   - Set your build command to:
     ```
     npm run build
     ```
   - Set your publish/output directory to:
     ```
     dist
     ```
   - This ensures the site is built before deployment and avoids the "Missing entry-point to Worker script or to assets directory" error.

## Troubleshooting

- **Error: Missing entry-point to Worker script or to assets directory**
  - This means the `dist` directory does not exist at deploy time.
  - Make sure your build command is `npm run build` (not just `npx wrangler deploy`).
  - The `wrangler.toml` file should have:
    ```toml
    [site]
    bucket = "./dist"
    ```
...
## Notes
- The `postbuild` script in `package.json` will automatically deploy your site after building.
- If you want to build without deploying, temporarily remove or comment out the `postbuild` script.
